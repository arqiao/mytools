...
| `faster-whisper` | Whisper 语音识别（CTranslate2 后端） | s3_subtitle.py |
| `opencc-python-reimplemented` | 繁体转简体 | s3_subtitle.py |
| `openpyxl` | Excel xlsx 解析 | s5_addon.py |

s2 步骤依赖 yitang 项目（`../yitang/src`），需确保 yitang 项目存在且包含：
- `yitang_wiki.py` — 飞书文档 blocks 获取与转换

s3/s4/s5 均为独立脚本，无跨项目依赖。
...
s3 字幕生成内置 Whisper 转写（通过 faster-whisper），需安装 `faster-whisper` 及对应的 CTranslate2。模型通过 `model_downloader.py` 自动管理。另外支持讯飞、飞书、阿里云、豆包 4 个云端引擎。

### LLM API（s4/s5 步骤）

字幕修订和信息拓展使用 LLM API（OpenAI 兼容接口）。支持 minimax/volcengine/dashscope/deepseek 等提供商，需在 `credentials.yaml` 中配置对应的 api_key。
...
addon:                            # 信息拓展配置
  chunk_size: 30000
  prompt_subtitle: "prompt-subtitle.md"
  prompt_discussion: "prompt-discussion.md"
  prompt_digest: "prompt-digest.md"
...
volcengine:
  api_key: "xxx"                       # s4/s5 LLM 调用（火山方舟）
  endpoint_id: "xxx"                   # 火山方舟 endpoint ID

dashscope:
  api_key: "xxx"                       # s4/s5 LLM 调用（通义千问）
...
aliyun:
  access_key_id: "xxx"                 # s3 阿里云转写
  access_key_secret: "xxx"
  app_key: "xxx"

doubao:
  access_key_id: "xxx"                 # s3 豆包转写
  access_key_secret: "xxx"
  app_id: "xxx"
```
...
| s5 | `{title}_addon.md` | 信息拓展完整报告 |
| s5 | `{title}_精华摘要.md` | 精华摘要（LLM 筛选） |
...
```bash
python src/s1w_panda.py       # 仅处理 config.yaml 中 source_type=panda 的任务
...
```bash
python src/s2_wiki.py        # 下载教学文档 + 写入飞书 wiki（依赖 yitang）
python src/s3_subtitle.py    # 多引擎字幕生成（需要 s1 产出的 .mp3）
python src/s4_srt_fix.py     # LLM 字幕修订（需要 _ori.srt 或 _wm.srt）
python src/s5_addon.py       # 信息拓展（需要修订后字幕 + _wiki.md）
```

s2 依赖 yitang 项目，运行前确保 `../yitang` 存在。s3/s4/s5 为独立脚本，无跨项目依赖。
