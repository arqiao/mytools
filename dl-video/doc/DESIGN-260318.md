...
| **会议标题** | API获取 | 通过 get-multi-record-info API，比页面解析更稳定 | sharing_id + cookies |
| **会议日期** | API获取 | 通过 get-multi-record-info API | sharing_id + cookies |
| **时间轴** | API获取 | 通过 get-multi-record-timeline API，避免点击UI | sharing_id + cookies |
| **会议纪要** | API获取 | 通过 get-multi-record-info API | sharing_id + cookies |
| **逐字稿** | API获取 | 通过 minutes/detail API，返回毫秒级时间戳 | meeting_id + recording_id + cookies |

**优化要点**：
- 浏览器只用于提取4个必需项（meeting_id、recording_id、视频URL、cookies）
- 所有文本数据通过API获取，避免不稳定的页面解析和UI点击
...
3. `fetch_meeting_data_via_api(meeting_id, recording_id, sharing_id, cookies)` — API 获取会议数据：
   - `GET /wemeet-tapi/v2/meetlog/public/record-detail/get-multi-record-timeline` → 时间轴
   - `GET /wemeet-tapi/v2/meetlog/public/record-detail/get-multi-record-info` → 会议信息（标题、日期、纪要）
   - 需要 sharing_id 作为 `auth_share_id` 参数
4. `fetch_transcript_via_api(meeting_id, recording_id, cookies)` — API 获取逐字稿：
   - `GET /wemeet-cloudrecording-webapi/v1/minutes/detail` → 逐字稿数据（带时间戳）
   - 需要 meeting_id 和 recording_id 参数
5. `parse_transcript_to_srt(transcript_lines)` — 将逐字稿转换为SRT格式（毫秒级时间戳）
6. `generate_abs_md(summary, timeline)` — 生成摘要markdown（纪要 + 时间轴）
7. `download_video(video_url, output_path, cookies)` — 流式下载视频（带进度显示）
8. `extract_audio(video_path, audio_path)` — ffmpeg 从视频提取音频
9. `safe_filename(title)` — 清理文件名中的非法字符（Windows兼容）
10. `process_tencent_meeting(task, config, creds)` — 主入口函数，协调上述流程
...
- API 获取会议数据比页面解析更稳定（避免点击UI元素的不确定性）
