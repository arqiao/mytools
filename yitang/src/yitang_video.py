...
class YitangLiveDownloader:
    def __init__(self):
        with open(CFG_DIR / "config-video.yaml", encoding="utf-8") as f:
        ...
        self.output_prefix = self.config.get("output_prefix", "AI落地Live_")
        self.output_dir = PROJECT_DIR / self.config.get("output_dir", "localscript")
    ...
    def export_chats(self, chats: list, output_path: Path, duration: int):
        ...
        for chat in chats:
            ...
            # 过滤无法用 GBK 编码的字符（主要是 emoji，Unicode 码点 >= 0x10000）
            def filter_non_gbk(text):
                if not text:
                    return ""
            ...
            rows.append({
                ...
                "发言人名称": filter_non_gbk(chat.get("name", "")),
                "发言内容": filter_non_gbk(chat.get("message", "")),
                "MBA": tags_info["MBA"],
                "年级": tags_info["年级"],
    ...
    def run(self, dl_video=True, dl_audio=True, dl_chat=True):
        livestreams = self.config.get("livestreams", [])
        if not livestreams:
            log.warning("config-video.yaml 中没有配置 livestreams")
            return
        ...
        for idx, item in enumerate(livestreams):
            ...
            try:
                ...
                # 判断标题是否属于"AI落地Live"系列
                if "AI落地Live" in title:
                    # 使用前缀+编号
                ...
                # 3. 下载音频
                if dl_audio and audio_url:
                    self.download_audio(audio_url, video_path)
