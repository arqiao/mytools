...
def load_config():
    with open(CFG_DIR / "config-srtfix.yaml", encoding="utf-8") as f:
...
def load_custom_dict(config):
    ...
    dict_file = config.get("fix", {}).get("custom_dict", "")
    if not dict_file:
        return {}
...
def run_llm_fix(entries, transcript_text, terms, custom_dict, config, creds,
    ...
    # 根据是否有逐字稿选择提示词
    if transcript_text:
        prompt_file = config.get("fix", {}).get("prompt", "prompt-srtfix-ref.md")
    else:
        prompt_file = config.get("fix", {}).get("prompt_noref", "prompt-srtfix-noref.md")
    ...
    chunk_size = config.get("fix", {}).get("chunk_size", 80)
...
def main():
    ...
    # 确定输入文件
    subtitle_path = args.subtitle or config["input"]["subtitle"]
    transcript_src = args.transcript or config["input"].get("transcript", "")

    # 解析字幕
    srt_path = Path(subtitle_path)
    if not srt_path.is_absolute():
        srt_path = PROJECT_DIR / "localscript" / subtitle_path
    if not srt_path.exists():
        log.error(f"字幕文件不存在: {srt_path}")
        return
    entries = parse_srt(srt_path)

    # 加载逐字稿（自适应：无/本地文件/飞书URL，MD文件名与SRT对齐）
    ...
    suffix = config["output"].get("suffix", "_fix")
    stem = srt_path.stem
